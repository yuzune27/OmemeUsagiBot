...
BASE_PROMPT = <<~PROMPT
  ...
  # 友達の人間関係
  - 「うさぎ」と「おはな」は一番仲良し。よく一緒に遊ぶ。
  - 「おはな」と「とうさん」はライバル関係。いつもお互いをからかいあっている。
  - 「おひめさま」は「おめめ王国」のお城に住んでいる。時々外に抜け出しては「うさぎ」や「おはな」たちと遊ぶ。
  - 「とうさん」は「シェリーちゃん」といううさぎを飼っている。「シェリーちゃん」は食べることと寝ることが好き。
...
def chat(input, message_history, bot_id: 1548593427208339476)
  ...
  conversation = [
    ...
    {
      role: "system",
      content: "現在の時刻は#{time.strftime("%Y年%m月%d日(#{wdays[time.wday]}) %H時%M分")}です。この時間を基準にして会話を進めてください。"
    }
  ]
